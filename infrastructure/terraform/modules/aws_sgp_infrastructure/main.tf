resource "aws_security_group" "microservice_ec2_sg" {
  name        = "${var.microservice_name}-ec2-sg"
...
resource "aws_instance" "microservice_ec2" {
  ...
  provisioner "local-exec" {
    command = "echo ${var.microservice_name} ansible_ssh_private_key_file=~/aws/key-pairs/sgp-ec2-key.pem id=${self.id} ansible_host=${self.public_ip} ansible_user=ubuntu aws_region=${var.region} aws_s3_bucket=${length(aws_s3_bucket.microservice_s3) > 0 ? aws_s3_bucket.microservice_s3[0].bucket : "N/A"} aws_dynamodb_table=${length(aws_dynamodb_table.microservice_dynamodb) > 0 ? aws_dynamodb_table.microservice_dynamodb[0].name : "N/A"} >> /etc/ansible/hosts"
...
resource "aws_s3_bucket" "microservice_s3" {
  ...
  bucket = "${var.microservice_name}-s3-${random_string.bucket_suffix.result}"
...
resource "aws_iam_role" "s3_dynamodb_full_access_role" {
  name = "${var.microservice_name}-s3_dynamodb_full_access_role"
...
resource "aws_iam_instance_profile" "s3_dynamodb_full_access_instance_profile" {
  name = "${var.microservice_name}-s3_dynamodb_full_access_instance_profile"
  role = aws_iam_role.s3_dynamodb_full_access_role.name
